...
namespace tcpip
{
        ...
        getFreeSocketPort()
    {
        ...
        if ( bind(sock, (struct sockaddr*) &self, address_len) < 0)
            BailOnSocketError("udp::Socket::getFreeSocketPort() Unable to bind socket");
        // get the assigned port with getsockname
        if ( getsockname(sock, (struct sockaddr*) &self, &address_len) < 0)
            BailOnSocketError("udp::Socket::getFreeSocketPort() Unable to get socket name");
...
#ifdef WIN32
		if( socket_ == -1 && init_windows_sockets_ && instance_count_ == 0 )
				WSACleanup();
...
#pragma warning(disable: 4548)
		...
		const
	{
		...
		if (r < 0)
			BailOnSocketError("udp::Socket::datawaiting @ select");
...
#pragma warning(pop)
		...
		set_blocking(bool blocking) 
	{
		...
		if(socket_ > 0 )
		{
		    ...
		    if (ioctlsocket(socket_, FIONBIO, &NonBlock) == SOCKET_ERROR)
				BailOnSocketError("udp::Socket::set_blocking() Unable to initialize non blocking I/O");
#else
		...
		const
	{
		...
		if (bytesReceived < 0)
		{
			...
			{
				// Non-blocking socket, no data available at the moment
				// Handle the case accordingly
			}
			else
			{
				// Other error occurred, handle it appropriately
				BailOnSocketError("udpip::Socket::recvAndCheck @ recvfrom");
			}
		}
